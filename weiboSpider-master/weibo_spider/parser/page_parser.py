    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                weibo.article_url = u'https://weibo.cn/comment/%s' % weibo.id
                ...
                weibo.spider_time = datetime.now().strftime('%Y-%m-%d %H:%M')
                weibo.retweet_num = footer['retweet_num']  # 转发数
                weibo.comment_num = footer['comment_num']  # 评论数
                #weibo.title =re.compile(r'[【] (.*?)[】]',weibo.content)
                weibo.title = weibo.content[0:20]
            else:
                weibo = None
